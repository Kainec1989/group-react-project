...
export default function Apod() {
  ...
  useEffect(() => {
    fetch(
      `https://api.nasa.gov/planetary/apod?api_key=gmKduGKQDPgsOVA5gMoA9CSQTSQJ38bSbMptoAM3`
    )
      ...
      .catch((error) => {
        console.error(
          "Opppps,There was an ERROR fetching the Astronomy picture of day",
          error
        );
      });
  }, []);
        ...
        {apodData ? (
          <div className=" p-9 ">
            <img
              src={apodData.url}
              alt={apodData.title}
              class="mx-auto outline outline-offset-2 outline-black-500 p-4 mb-4  "
            />

            <h2 className="text-2xl text-center font-semibold mb-9 ">
              Image: {apodData.title}
            </h2>
